...
func (as *ArticleService) GetPageBy(uid, page int, sort string) models.Response {
	...
	if sort, err := as.parseSortValue(sort); err != nil {
		...
		articles, _ := as.repo.GetArticlesWithState(uid, page, 0, sort)
		...
		wg.Wait()
		if sort.Key == "views" {
			sort2.Slice(articles, func(i, j int) bool {
				if sort.Value == "desc" {
...
func (as *ArticleService) GetCategoryItemArticlesPageBy(uid, cid, page int, sort string) models.Response {
	//判断用户跟分类是否存在
	if resp, found := as.CheckUserExist(uid); !found {
		return resp
	}
		...
		if category, found := categoryRepo.GetCategoryById(categoryItem.CategoryID); !found {
			...
			if sort, err := as.parseSortValue(sort); err != nil {
				...
				if category.Name == "个人分类" {
					articles, _ := as.repo.GetCategoryItemArticlesPage(uid, cid, page, sort)
					resp["articles"] = articles
					resp["count"] = as.repo.GetCountByCid(uid,cid)
				} else {
					articles, _ := as.repo.GetArchizeArticlesPage(uid, page, categoryItem.Name, sort)
					resp["articles"] = articles
